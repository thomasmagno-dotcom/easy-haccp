...
import type { OutputHazardAssignment } from "@/lib/types";
...
export function OutputRiskMatrix({ assignments }: Props) {
  const cellHazards: Record<string, OutputHazardAssignment[]> = {};
      ...
      <div className="min-w-[500px]">
        <div className="flex">
          ...
          {LIKELIHOOD_LEVELS.map((lik) => (
            <div key={lik.value} className="flex-1 text-center px-1 pb-1">
        ...
        <div className="flex mt-2">
          <div className="w-28 shrink-0" />
          <div className="flex-1 text-center text-xs text-neutral-500 font-medium">Likelihood →</div>
        </div>

        <div className="flex gap-4 mt-4 justify-center text-[10px]">
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-green-100 border border-green-300" />Low (1-3)</span>
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-yellow-100 border border-yellow-300" />Medium (4-6)</span>
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-orange-200 border border-orange-300" />High (8-9) — Significant</span>
          <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-sm bg-red-200 border border-red-300" />Critical (12-16) — Significant</span>
        </div>
      </div>
    </div>
  );
