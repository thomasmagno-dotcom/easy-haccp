...
import type { OutputCcpData } from "@/lib/types";
...
interface FormState {
  ...
  limits: Array<{ parameter: string; minimum: string; maximum: string; target: string; unit: string; scientificBasis: string }>;
  monitoring: Array<{ what: string; how: string; frequency: string; who: string; recordForm: string }>;
  corrective: Array<{ deviation: string; immediateAction: string; productDisposition: string; rootCauseAnalysis: string; preventiveAction: string; responsiblePerson: string; recordForm: string }>;
  verification: Array<{ activity: string; frequency: string; responsiblePerson: string; method: string; recordReference: string }>;
}

function initForm(ccpData: OutputCcpData | null): FormState {
  if (!ccpData) {
    return {
      hazardDescription: "",
  ...
  return {
    ...
    limits: ccpData.criticalLimits.length > 0
      ? ccpData.criticalLimits.map((l) => ({ parameter: l.parameter, minimum: l.minimum || "", maximum: l.maximum || "", target: l.target || "", unit: l.unit || "", scientificBasis: l.scientificBasis || "" }))
      : [{ parameter: "", minimum: "", maximum: "", target: "", unit: "", scientificBasis: "" }],
    monitoring: ccpData.monitoringProcedures.length > 0
      ? ccpData.monitoringProcedures.map((m) => ({ what: m.what, how: m.how, frequency: m.frequency, who: m.who, recordForm: m.recordForm || "" }))
      : [{ what: "", how: "", frequency: "", who: "", recordForm: "" }],
    corrective: ccpData.correctiveActions.length > 0
      ? ccpData.correctiveActions.map((c) => ({ deviation: c.deviation, immediateAction: c.immediateAction, productDisposition: c.productDisposition, rootCauseAnalysis: c.rootCauseAnalysis || "", preventiveAction: c.preventiveAction || "", responsiblePerson: c.responsiblePerson, recordForm: c.recordForm || "" }))
    ...
    verification: ccpData.verificationProcedures.length > 0
      ? ccpData.verificationProcedures.map((v) => ({ activity: v.activity, frequency: v.frequency, responsiblePerson: v.responsiblePerson, method: v.method || "", recordReference: v.recordReference || "" }))
      : [{ activity: "", frequency: "", responsiblePerson: "", method: "", recordReference: "" }],
  };
}

export function OutputCcpDetailsSection({ planId, outputId, ccpData }: Props) {
  ...
  function addArrayItem<K extends "limits" | "monitoring" | "corrective" | "verification">(arrayKey: K) {
    ...
    setForm((prev) => ({ ...prev, [arrayKey]: [...prev[arrayKey], ...emptyItems[arrayKey]] }));
  }

  async function save() {
    setSaving(true);
    const res = await fetch(`/api/plans/${planId}/output-ccps`, {
      method: "POST",
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <Textarea value={form.hazardDescription} onChange={(e) => updateField("hazardDescription", e.target.value)} rows={2} placeholder="e.g., Biological: Salmonella, E. coli" className="mt-1" />
        </div>
        <div>
          <Label className="text-xs">Control Measure</Label>
          <Textarea value={form.controlMeasureDescription} onChange={(e) => updateField("controlMeasureDescription", e.target.value)} rows={2} placeholder="e.g., Temperature control at ≥74°C" className="mt-1" />
        </div>
      </div>

      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-sm">Critical Limits</CardTitle></CardHeader>
      ...
      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-sm">Monitoring Procedures</CardTitle></CardHeader>
        <CardContent className="space-y-3">
      ...
      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-sm">Corrective Actions</CardTitle></CardHeader>
        <CardContent className="space-y-3">
      ...
      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-sm">Verification Procedures</CardTitle></CardHeader>
        <CardContent className="space-y-3">
